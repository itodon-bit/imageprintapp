...
3. 「画像を印刷」ボタンでシステムの印刷ダイアログを開き、Wi-Fi接続されたプリンタへ選択範囲の画像を印刷
4. 「選択範囲を文字認識」ボタンで、選択した範囲だけをOCR(文字認識)してテキスト化
   - 認識結果は編集欄に表示され、誤字があれば手直しできる
   - 「テキストを印刷」ボタンで、そのテキストを紙のレイアウトに整形してプリンタへ送信
...
- OCR(文字認識): Google の **ML Kit**(日本語対応モデル)を使用
  - 選択範囲を切り出したBitmapを渡すと、端末上で文字認識が行われる(初回のみモデルのダウンロードが必要なためインターネット接続が必要)
  - 認識結果はテキストとして取得し、`StaticLayout`で紙のレイアウトに描画した画像に変換してから、画像印刷と同じ`PrintHelper`で印刷する
